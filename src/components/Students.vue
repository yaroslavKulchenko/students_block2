    ...
    <input type="text" v-model="piece">
            <table v-for="Student in students" v-bind:key="Student._id">
                <template v-if="Student._id==editId">
                    ...
                    <td>{{Student.group}}</td>
                    <td> <button v-on:click="update(editId)" value="update">Изменить</button></td>
                </template> 
                <template v-else>   
                ...
                <td>{{Student.group}}</td>
                    ...
                    <td><button v-on:click="removeStudent(Student._id)">Удалить</button></td>
                    <td><button v-on:click="replaceForm(Student._id)">Изменить</button></td>
                    <td><a href="#" @click.prevent="removeStudent(Student._id)" v-show="Student.group==getCurrentUser.group">Видалити</a></td>
                </tr> 
                <br>
                </template>
            </table>
